...
class ArchivadorBienestar extends Model
{
    protected $table = "cb_archivador";
    ...
    protected function setArchivo($request)
    {
        ...
        $archivo = new ArchivadorBienestar;
    ...
    protected function actualizarArchivo($request)
    {
        ...
        switch ($request->campo) {
            case 'fecha':
                $archivo = ArchivadorBienestar::find($request->id);
                if (!is_null($archivo)) {
    ...
    {
        $archivos = DB::table('cb_archivador as ca')
            ->select([
                'ca.id',
